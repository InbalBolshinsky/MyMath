const checkAndUpdateTrophies = async (user) => {
    ...
    if (latestSession) {
        ...
        console.log('Parsed correct answers:', correct);
        console.log('Parsed session score:', sessionScore);
        console.log('Current high score in database:', user.highScore);

        if (sessionScore > (user.highScore || 0)) {
            user.highScore = sessionScore;
            console.log('New high score set:', user.highScore);
        } else {
            console.log('High score remains the same:', user.highScore);
        }
        ...
        if (correct >= 7 && !user.trophies.includes('7 in a row')) {
            ...
            console.log('Achievement unlocked: 7 in a row');
        }
